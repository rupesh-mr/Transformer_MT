...
class EncoderLayer(nn.Module):
    def __init__(self, d_model, nhead, d_ff, use_moe=False):
        super().__init__()
        self.self_attn = MultiHeadAttention(d_model, nhead)
        self.use_moe = use_moe
        self.ffn = MoE(d_model, d_ff//4, num_experts=4, top_k=2) if use_moe else FeedForward(d_model, d_ff)
    ...
    def forward(self, x, mask, return_attn=False):
        ...
        if self.use_moe:
            ...
            moe_output = self.ffn(self.norm2(x))
            lb_loss = 0.0

        x = x + self.dropout(moe_output)
        return x, lb_loss if not return_attn else (x, attn, lb_loss)
...
class DecoderLayer(nn.Module):
    ...
    def forward(self, x, mem, tgt_mask, mem_mask, return_attn=False):
        if return_attn:
            sa_out, self_attn_weights = self.self_attn(
                self.norm1(x), self.norm1(x), self.norm1(x), tgt_mask, return_weights=True
            )
            x = x + self.dropout(sa_out)

            ca_out, cross_attn_weights = self.cross_attn(
                self.norm2(x), mem, mem, mem_mask, return_weights=True
            )
        ...
        if self.use_moe:
            ffn_output, _ = self.ffn(self.norm3(x))  # load-balance loss ignored in decoder
        else:
...
def build_encoder_layers(num_layers, d_model, nhead, d_ff, moe_condition):
    layers = []
    for i in range(num_layers):
        use_moe = moe_condition(i, num_layers)
        layer = EncoderLayer(d_model, nhead, d_ff, use_moe=use_moe)
        layers.append(layer)
    return nn.ModuleList(layers)

def build_decoder_layers(num_layers, d_model, nhead, d_ff, moe_condition):
    layers = []
    for i in range(num_layers):
        use_moe = moe_condition(i, num_layers)
        layer = DecoderLayer(d_model, nhead, d_ff, use_moe=use_moe)
        layers.append(layer)
    return nn.ModuleList(layers)
